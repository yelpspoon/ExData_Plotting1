...
##------------ Plots -----------##
# Plot 1 (from 2)
plot1<- ggplot(DT, aes(Date, Global_active_power)) + 
  geom_line() + xlab("") +  
...
plot2<- ggplot(DT, aes(Date)) + 
  ...
  theme(panel.background = element_rect(fill = "white"),
        ...
        legend.position = c(.747,.84) ) 

# Plot 3 
plot3<- ggplot(DT, aes(Date, Voltage)) + 
  geom_line() +
...
# Plot 4
plot4<- ggplot(DT, aes(Date,Global_reactive_power)) + 
  geom_line() +
...
source("/home/dlux/R/multiplot.R")
